...
- APIキーは **環境変数 `ANTHROPIC_API_KEY` のみ** から読み込みます。ソースコードや設定ファイルには書かないでください
- キーをリポジトリにコミットしない(誤ってコミットした場合は即座に無効化して再発行)
- 将来 Web 版や iPhone アプリ版を作る場合も、**キーをフロントエンド(ブラウザやアプリ内)に置くことは厳禁**。
  必ず自前のバックエンド(APIプロキシ)を経由し、キーはサーバー側だけに保持すること
